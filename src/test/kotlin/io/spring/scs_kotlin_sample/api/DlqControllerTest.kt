...
@DirtiesContext
internal class DlqControllerTest : ScsKotlinSampleApplicationTests() {
    ...
    @Test
    fun sendDlqException() {
        mockMvc.post("/api/dlq/custom") {
    ...
    @Test
    fun sendGeneralDlqException() {
        mockMvc.post("/api/dlq/general") {
            this.contentType = MediaType.APPLICATION_JSON
            this.content = objectMapper.writeValueAsString(Factories.getErrorDto())
        }.andExpect {
            this.status { isOk() }
